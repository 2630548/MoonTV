...
function PlayPageClient() {
 ...
 class CustomHlsJsLoader extends Hls.DefaultConfig.loader {
  ...
  useEffect(() => {
        ...
        customType: {
          m3u8: function (video: HTMLVideoElement, url: string) {
            ...
            const hls = new Hls({
              ...
              /* 缓冲/内存相关 */
              maxBufferLength: 900, // 前向缓冲最大 30s，过大容易导致高延迟
              maxMaxBufferLength: 9000,
              backBufferLength: 900, // 仅保留 30s 已播放内容，避免内存占用
    ...
    console.log(new Date()+`[加载] SN=${data.frag.sn}, URL=${data.frag.url}`);
});
hls.on(Hls.Events.FRAG_LOADED, (event, data) => {
    console.log(new Date()+`[完成] SN=${data.frag.sn}`);
});
